...
class GameScreen:
    ...
    def run(self):
        ...
        running = True
        show_game_over_screen = False  # controle para game over não endless
        ...
        while running:
                ...
                if self.paused:
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        if self.pause_buttons and self.pause_buttons["continuar"].collidepoint(mouse_pos):
                            ...
                            running = False
                            return "nick_difficulty"  # voltar para tela de dificuldade
            ...
            if not self.game_over and not self.victory and not self.paused:
                ...
                if self.game.base_hp <= 0:
                    ...
                    if self.difficulty == "endless":
                        ...
                        self.ranking_screen = True  # mostrar ranking para endless
                    else:
                        show_game_over_screen = True  # mostrar game over normal
            ...
            if self.paused:
                self.pause_buttons = draw_pause_menu(
                    self.screen, self.screen.get_width(), self.screen.get_height(),
                    pygame.font.SysFont(None, 72), pygame.font.SysFont(None, 36)
                )

            elif self.game_over:
                # Se for endless, mostrar ranking
                if self.difficulty == "endless":
                    ranking_buttons = draw_ranking_screen(
                        self.screen, self.screen.get_width(), self.screen.get_height(),
                        pygame.font.SysFont(None, 72), pygame.font.SysFont(None, 36), self.ranking
                    )
                ...
                # Se for game over normal, mostrar tela game over
